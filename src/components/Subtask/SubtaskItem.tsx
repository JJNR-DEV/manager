...
const SubtaskItem: React.FC<Props> = ({
  ...
  const handleCheck = () => {
    tasksCollection(firebase, parentUserOrigin)
      .doc(parentId)
    ...
    latestUpdate(firebase, parentUserOrigin, parentId);
  };

  const handleDelete = () => {
    swal({
      title: `Are you sure you want to erase "${subtask.name}"?`,
  ...
  return (
    ...
    >
      <div className='nameState'>
        ...
        />
        <span className={ subtask.done ? `${parentCategory} customCheckbox` : 'customCheckbox' } onClick={handleCheck}>
          {subtask.done ? '\u2713' : ''}
        </span>

        <span className={ subtask.done ? 'concludedElement subtaskName' : 'subtaskName' }>
          {subtask.name}
        </span>
      </div>
      <div className='priceErase'>
        {subtask.price !== null && (
          <span className={ subtask.done ? 'concludedElement subtaskPrice' : 'subtaskPrice' }>
            {subtask.price}kr
          </span>
        )}
        {
...
export default withFirebase(SubtaskItem);
